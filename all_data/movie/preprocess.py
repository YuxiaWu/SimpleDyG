...
with open('./ia-movielens-user2tags-10m.edges') as f:
    ...
    for l in lines:
        ...
        links.append((node_idx[x],node_idx[y], timestamp ,t, year_month )) # add t
...
for (a, b, time, ori_time,_) in links: # add ori_time
    ...
    slices_links[slice_id].add_edge(a,b, weight= e, date=ori_time)
...
with open('ML_10M_13_ori.csv', 'w') as f:
...
data = pd.read_csv('ML_10M_13_ori.csv')
data['time_diff'] = (data['ori_time'] - data['ori_time'].min()) / (data['ori_time'].max() - data['ori_time'].min())
data = data.sort_values(by=['time_diff'])
data.to_csv('ML_10M_13.csv', index=False)
# print the max time for each timestamp
for i, group in data.groupby('timestamp'):
